...
public class RSAUtils {
    ...
    public static void main(String[] args) {
        ...
        String time = "2016-12-30";
//        Keys.PRIVATEKEY.getValue();
        try {
            System.out.println(Keys.PRIVATEKEY.getValue());
//            String token = Base64.encodeBase64String(encryptByPrivateKey((id + time).getBytes()));
            System.out.println("token--- " + Base64.encodeBase64String(encryptByPrivateKey((id + time).getBytes())));

            String token = Base64.encodeBase64URLSafeString(encryptByPrivateKey((id + time).getBytes()));

            System.out.println("token--2 " + token);
            System.out.println();
//            decryptByPublicKey(Base64.decodeBase64(token.getBytes()),Keys.PUBLICKEY)
            System.out.println("明文:" + new String(RSAUtils.decryptByPublicKey(Base64.decodeBase64(token.getBytes()))));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
